%   Search algorithm to find optimal set of N 2-outcome projective measurements
...
%   sets of N 2-outcome projective measurements.
%   
...
function [opt_M, eta] = search_qubit_proj(rho_AB, N)
%search_qubit_planproj Calculates a lower bound for the critical visibility
%   eta of a two-qubit quantum state rho_AB subjected to N 2-outcome
...
%   a set of N 2-outcome projective measurements and the objective
...
np = 2*N;           % total number of parameters in the parameter vector x
...
[a,fval,flag] = fminsearch(@(x)wnr_qubitproj(x,rho_AB,N),x0,options);
% calls fminsearch to optimize over function wnr_qubitproj given initial
...
for i=1:N
    ...
    opt_M(:,:,i,1) = (1/2)*(eye(dA)+sin(a(t))*cos(a(t+1))*XX+sin(a(t))*sin(a(t+1))*YY+cos(a(t))*ZZ);
    opt_M(:,:,i,2) = eye(dA) - opt_M(:,:,i,1);
    t = t + 2;
end

% upper bound for the critical visibility of rho_AB subjected to N 2-outcome projective
% measurements is the optimal value of the objective function in the optimization
eta = fval;

end

function eta = wnr_qubitproj(x,rho_AB,N)
%wnr_qubitproj Is the function called by fminsearch. It constructs sets of 
%   N 2-outcome projective measurements M_ax from the parameter vector x.
...
for i=1:N
    M_ax(:,:,i,1) = (1/2)*(eye(dA)+sin(x(t))*cos(x(t+1))*XX+sin(x(t))*sin(x(t+1))*YY+cos(x(t))*ZZ);
    M_ax(:,:,i,2) = (1/2)*(eye(dA)-sin(x(t))*cos(x(t+1))*XX-sin(x(t))*sin(x(t+1))*YY-cos(x(t))*ZZ);
    for j=1:k
