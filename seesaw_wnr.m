...
%   critical visibility of the input quantum state when subjected to
...
%   the see-saw. The value eta is a upper bound for the critical visibility
%   of the state rho_AB when subjected to arbitrary sets of N k-outcome
%   measurements.
%
...
while gap>precision
    ...
    % fixed state rho_AB and set of measurements M -- see WNR_sdp1
    [F_ax, J, flag] = WNR_sdp1(M_ax,rho_AB);
    
    if flag.problem~=0
        WNR_sdp1_problem = flag.problem % will print flag on screen if SDP fails
    ...
    % -- see WNR_sdp2
    [M_ax, flag] = WNR_sdp2(F_ax,rho_AB);  
    
    if flag.problem~=0
        WNR_sdp2_problem = flag.problem % will print flag on screen if SDP fails
        info             = flag.info % will print flag info on screen if SDP fails
    end
...
[eta, ~] = wnr_ste(rho_AB,M_ax); % calculates the critical visibility eta of the input state rho_AB and optimal set of measurements opt_M
...
%RandomPOVM Generates a random POVM tensor given dimension d and number of
%   outcomes k
%
...
function [F_ax, J, flag] = WNR_sdp1(M_ax,rho_AB)
%WNR_sdp1 Is the dual formulation of the SDP that calculates the white noise
%   robustness of steering J of a quantum state rho_AB subjected to local set
...
for i=1:N
    for j=1:k
        ...
        trF_trsig = trF_trsig + trace(F_ax(:,:,i,j))*trace(sig_ax);
    end
end
...
for l=1:k^N
    ...
    F = F + [(1-(1/dB)*trF_trsig)*eye(dB)+U>=0]; % SDP constraint
end

J = -1 - tr_Fsig + (1/dB)*trF_trsig; % white noise robustness of steering

% maximizes white noise robustness of steering J subjected to constraints F
...
function [M_ax, flag] = WNR_sdp2(F_ax,rho_AB)
%WNR_sdp2 Finds the set M of N k-outcome of measurements that, when locally
...
function [eta, solution] = wnr_ste(rho_AB, M_ax)
...
tr_sum_sig = trace(sum(sig_loc,3));
...
for i=1:N
    for j=1:k
        ...
        F = F + [sig_ax+((tr_sum_sig-1)/dB)*trace(sig_ax)*eye(dB)==uns_ax]; % SDP constraint
    end
end

J = tr_sum_sig - 1;

% minimizes the amount of white noise that is necessary to apply to
...
J   = double(J);
eta = 1/(1+J); % calculates critical visibility eta as a function of the white noise robustness J
